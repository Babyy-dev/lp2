...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (carousel) {
        ...
        // Enhanced arrow button effects
        const carouselButtons = document.querySelectorAll('.magical-carousel-btn');
        carouselButtons.forEach(button => {
            button.addEventListener('mouseenter', function() {
                this.style.animation = 'arrow-bounce 0.6s ease-in-out infinite, arrow-glow 1s ease-in-out infinite';
                createArrowSparkles(this);
            });
            
            button.addEventListener('mouseleave', function() {
                this.style.animation = 'arrow-glow 2s ease-in-out infinite';
            });
            
            button.addEventListener('click', function() {
                createArrowClickEffect(this);
            });
        });
    }
    ...
    function createArrowSparkles(button) {
        ...
        for (let i = 0; i < 5; i++) {
            setTimeout(() => {
                ...
                sparkle.innerHTML = ['⭐', '✨', '💫', '🌟', '💥'][i];
                sparkle.style.position = 'fixed';
                sparkle.style.left = (rect.left + rect.width/2 + (Math.random() - 0.5) * 40) + 'px';
